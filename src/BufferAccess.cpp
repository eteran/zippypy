...
#define CATCH_ACCESS_VIOLATION(func)                                        \
	catch (const std::exception &) {                                        \
		throw;                                                              \
	}                                                                       \
	catch (...) {                                                           \
		THROW("Exception in " #func " " << (void *)m_p << "+" << m_offset); \
	}
...
ObjRef AccessBuffer::readInt(int numBytes) {
	...
	case 1:
		return readNum<char>();
	case 2:
		return readNum<short>();
	case 4:
		return readNum<int>();
	case 8:
		return readNum<int64_t>();
	default:
		THROW("can't readInt " << numBytes);
	}
}
...
void AccessBuffer::writeInt(int numBytes, const ObjRef v) {
	switch (numBytes) {
	case 1:
		return writeNum<char>(v);
	//case 2: return writeNum<short>(v); # TODO - Fix, Not compiling
	case 4:
		return writeNum<int>(v);
	case 8:
		return writeNum<int64_t>(v);
	default:
		THROW("can't writeInt " << numBytes);
	}
}
