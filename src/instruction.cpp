...
bool OpImp::compare(const ObjRef &lhsref, const ObjRef &rhsref, int op) {
	...
	if (lhs->type == rhs->type) {
		switch (lhs->type) {
		case Object::INT:
			return compareType<IntObject>(lhs, rhs, op);
		case Object::BOOL:
			return compareType<BoolObject>(lhs, rhs, op);
		case Object::STR:
			return compareStrType<StrObject>(lhs, rhs, op);
		case Object::USTR:
			return compareStrType<UnicodeObject>(lhs, rhs, op);
		case Object::TUPLE:
		case Object::LIST:
			return compareList((ListObject *)lhs, (ListObject *)rhs, op);
		case Object::FLOAT:
			return compareType<FloatObject>(lhs, rhs, op);
...
public:
	struct Guard {
		Guard()
			: m_tracker(nullptr), m_obj(nullptr) {}
		Guard(RecurionTracker *tracker, Object *o)
			: m_tracker(tracker), m_obj(o) {
	...
	for (uint i = 0; i < s.size(); ++i) {
		...
		if (c >= 0x20 && c <= 0x7e && c != '\'' && c != '"')
			...
			case '\\':
				...
				break;
			default:
				e += ((std::basic_ostringstream<TC> &)(std::basic_ostringstream<TC>() << 'x' << std::hex << (int)c)).str();
				break;
			}
		}
	}
...
void print_recurse(const ObjRef &vref, std::ostream &out, RecurionTracker &tracker, bool repr) {
	...
	case Object::INT:
		...
		break;
	case Object::BOOL:
		out << (((BoolObject *)v)->v ? "True" : "False");
		break;
	case Object::STR:
		out << printStr(((StrObject *)v)->v, repr, 0);
		break;
	case Object::USTR:
		out << utf8FromWstr(printStr(((UnicodeObject *)v)->v, repr, 'u'));
		break;
	case Object::FLOAT:
		out << ((FloatObject *)v)->v;
		break;
	case Object::CODE:
		out << ((CodeObject *)v)->m_co.co_name;
		break;
	case Object::MODULE:
		out << ((ModuleObject *)v)->m_name;
		break;
...
ObjRef OpImp::add(const ObjRef &lhsref, const ObjRef &rhsref) {
	...
	if (lhs->type == rhs->type) {
		switch (rhs->type) {
		case Object::INT:
			return addType<IntObject>(lhs, rhs);
		case Object::FLOAT:
			return addType<FloatObject>(lhs, rhs);
		case Object::STR:
			return addType<StrObject>(lhs, rhs);
		case Object::USTR:
			return addType<UnicodeObject>(lhs, rhs);
		}
	}
	if (lhs->type == Object::STR && rhs->type == Object::USTR) {
...
ObjRef OpImp::mult(const ObjRef &lhsref, const ObjRef &rhsref) {
	...
	if (lhs->type == rhs->type) {
		switch (rhs->type) {
		case Object::INT:
			return multType<IntObject>(lhs, rhs);
		case Object::FLOAT:
			return multType<FloatObject>(lhs, rhs);
		}
	}
...
ObjRef OpImp::sub(const ObjRef &lhsref, const ObjRef &rhsref) {
	...
	if (lhs->type == rhs->type) {
		switch (rhs->type) {
		case Object::INT:
			return subType<IntObject>(lhs, rhs);
		case Object::FLOAT:
			return subType<FloatObject>(lhs, rhs);
		}
...
ObjRef OpImp::div(const ObjRef &lhsref, const ObjRef &rhsref) {
	...
	if (lhs->type == rhs->type) {
		switch (rhs->type) {
		case Object::INT:
			return divType<IntObject>(lhs, rhs);
		case Object::FLOAT:
			return divType<FloatObject>(lhs, rhs);
		}
	} else if ((lhs->type == Object::FLOAT && rhs->type == Object::INT)) {
		FloatObject floatRhs((float)((IntObject *)rhs)->v);
...
ObjRef OpImp::uminus(const ObjRef &argref) {
	...
	switch (arg->type) {
	case Object::INT:
		return minusType<IntObject>(arg);
	case Object::FLOAT:
		return minusType<FloatObject>(arg);
	}
...
static int64_t intLen(const ObjRef &argref) {
	...
	case Object::TUPLE:
		return lenType<TupleObject>(arg);
	case Object::DICT:
		return lenType<DictObject>(arg);
	case Object::STRDICT:
		return lenType<StrDictObject>(arg);
	case Object::STR:
		return lenType<StrObject>(arg);
	case Object::USTR:
		return lenType<UnicodeObject>(arg);
	default:
...
ObjRef OpImp::int_(const ObjRef &arg) {
	...
	switch (arg->type) {
	case Object::STR:
		i = lexical_cast(static_pcast<StrObject>(arg)->v);
		break;
	case Object::USTR:
		i = lexical_cast(static_pcast<UnicodeObject>(arg)->v);
		break;
	case Object::BOOL:
		i = static_pcast<BoolObject>(arg)->v ? 1 : 0;
		break;
	case Object::FLOAT:
		i = (int64_t)(static_pcast<FloatObject>(arg)->v);
		break;
	default:
...
ObjRef OpImp::bool_(const ObjRef &arg) {
	...
	switch (arg->type) {
	case Object::STR:
		b = boolFromStr(static_pcast<StrObject>(arg)->v);
		break;
	case Object::USTR:
		b = boolFromStr(static_pcast<UnicodeObject>(arg)->v);
		break;
	case Object::INT:
		b = (static_pcast<IntObject>(arg)->v == 0) ? false : true;
		break;
	case Object::FLOAT:
		b = (static_pcast<FloatObject>(arg)->v == 0.0) ? false : true;
		break;
	case Object::NONE:
		b = false;
		break;
...
int64_t binOp(int64_t a, int64_t b, uchar op) {
	...
	case BINARY_RSHIFT:
	case INPLACE_RSHIFT:
		return (*(uint64_t *)&a) >> b; // avoid sign extension
	case BINARY_LSHIFT:
	case INPLACE_LSHIFT:
		return a << b;
	}
